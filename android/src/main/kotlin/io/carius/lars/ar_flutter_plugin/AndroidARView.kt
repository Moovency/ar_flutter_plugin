        ...
        override fun onMethodCall(call: MethodCall, result: MethodChannel.Result) {
            ...
            when (call.method) {
                ...
                "getCameraOrientation" -> {
                    var data = arSceneView.arFrame?.getCamera()?.getDisplayOrientedPose()?.getRotationQuaternion();
                    result.success(data);
                }
                "startRecording" -> {
                    // retrieving path from function call
                    val path = call.argument<String>("path");
                    
                    var uri = Uri.fromFile(File(context.getFilesDir(), path));
                    var file_path = uri.getPath();
                    
                    // retrieving session to ini recording config
                    var session = arSceneView.session ?: Session(context);
                    
                    // recording config init
                    val recordingConfig = RecordingConfig(session).setMp4DatasetFilePath(file_path).setRecordingRotation(90);
                    
                    // starting recording
                    arSceneView.session?.startRecording(recordingConfig);
                    
                    result.success(file_path);
                }
                "stopRecording" -> {
                    arSceneView.session?.stopRecording();
                }
                "enableLogoTracking" -> {
                    // retrieving session to ini recording config
                    var session = arSceneView.session ?: Session(context);
                    
                    // creating image database from session
                    val imageDatabase = AugmentedImageDatabase(session);
                    
                    // retrieving image
                    val img_path = "aruco.png";
                    var ims = context.getAssets().open(img_path);
                    val bitmap = BitmapFactory.decodeStream(ims);
                    
                    // adding image to database
                    val logoIndex = imageDatabase.addImage("moovency_logo", bitmap);
                ...
                "isLogoTracked" -> {
                    ...
                    if(updatedAugmentedImages != null) {
                        ...
                        for (img in updatedAugmentedImages) {
                            if (img.trackingState == TrackingState.TRACKING && img.index == index) {
                                trigger = true;
                ...
                "getTrackingStatus" -> {
                    ...
                    when(camera?.getTrackingState()) {
                        TrackingState.TRACKING -> result.success("TRACKING")
                        TrackingState.PAUSED -> result.success("PAUSED")
                        TrackingState.STOPPED -> result.success("STOPPED")
                        else -> result.success("ERROR UNKNOWN TRACKING STATE")
                    }
                }
                "getTrackingError" -> {
                    val camera = arSceneView.arFrame?.camera
                    when(camera?.getTrackingFailureReason()) {
                        TrackingFailureReason .BAD_STATE -> result.success("BAD_STATE")
                        TrackingFailureReason .CAMERA_UNAVAILABLE -> result.success("CAMERA_UNAVAILABLE")
                        TrackingFailureReason .EXCESSIVE_MOTION -> result.success("EXCESSIVE_MOTION")
                        TrackingFailureReason .INSUFFICIENT_FEATURES -> result.success("INSUFFICIENT_FEATURES")
                        TrackingFailureReason .INSUFFICIENT_LIGHT -> result.success("INSUFFICIENT_LIGHT")
                        TrackingFailureReason .NONE -> result.success("NONE")
                        else -> result.success("UNKNOWN_TRACKING_ERROR")
                    }
                }
                "getQuaternionFromLogo" -> {
                    val index = call.argument<Int>("index");
                        ...
                        for (img in updatedAugmentedImages) {
                            if (img.trackingState == TrackingState.TRACKING && img.index == index) {
                                ...
                                when (img.trackingMethod) {
                                    ...
                                    AugmentedImage.TrackingMethod.FULL_TRACKING -> {
                                        var pose = img.getCenterPose();
                                        
                                        result.success(pose.getRotationQuaternion());
                                    }
                                    AugmentedImage.TrackingMethod.NOT_TRACKING -> {
                                        // NOPE NOPE NOPE
                                    }
                            ...
                            if (img.trackingState == TrackingState.TRACKING && img.index == index) {
                                ...
                                when (img.trackingMethod) {
                                    ...
                                    AugmentedImage.TrackingMethod.FULL_TRACKING -> {
                                        ...
                                        result.success(pose.getTranslation());
                                    }
                                    AugmentedImage.TrackingMethod.NOT_TRACKING -> {
                                        // NOPE NOPE NOPE
                                    }
                ...
                "getRecordLength" -> {
                    /*RecordingConfig config = RecordingConfig(arSceneView.session?);
                    val arg = call.argument<String>("path");
                    Uri path = Uri.parse(arg);
                    MediaPlayer mp = MediaPlayer.create(this, path);
